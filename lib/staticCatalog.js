...
// ---------------------------------------------------------------------------
// These two are deliberately re-declared here rather than imported from
// lib/amazon.js. That module reaches for fs, os and crypto to run its caches;
// this one is a pure reader of a JSON file, and keeping it free of those
// imports means it can never accidentally drag Node built-ins into a browser
// bundle. Six duplicated lines is a fair price for that guarantee.
// ---------------------------------------------------------------------------
...
function hydrate(p, cat) {
  return {
    ...p,
    cat,
    image: p.image || placeholderImage(p.title),
    url: withTag(p.url || "https://www.amazon.com/dp/" + p.asin),
